...
class GroupVaultDocAddScreen extends ConsumerStatefulWidget {
  const GroupVaultDocAddScreen({
  ...
  @override
  ConsumerState<GroupVaultDocAddScreen> createState() {
    return _GroupVaultDocAddScreenState();
  }
}

class _GroupVaultDocAddScreenState
    extends ConsumerState<GroupVaultDocAddScreen> {
    ...
    try {
      ...
      if (normalized == 'files') {
        ...
        if (pickedBytes == null || pickedBytes.isEmpty) {
          setState(() {
            _errorMessage = 'Could not read selected file. Please choose another file.';
          });
          return;
        }
        bytes = pickedBytes;
      ...
      if (bytes.isEmpty) {
        setState(() {
          _errorMessage = 'Selected image is empty. Please choose another file.';
        });
        return;
      }
      ...
      children: <Widget>[
        ...
        TextField(
          ...
          decoration: const InputDecoration(
            ...
            hintText: 'e.g. Group Passport Copy, Utility Bill Feb 2026',
          ),
        ),
        const SizedBox(height: 10),
